...
@conversation_bp.route('/id/<string:id>/words/review', methods=['POST'])
async def conversation_review_words(id):
  ...
  # Sanitize items: only forward the fields the LLM needs.
  payload_items = []
  for it in items:
  ...
  try:
    ...
    # Ensure we always return as many items as we got, in order.
    ...
    if len(out_items) != len(payload_items):
      ...
      # Align by index, pad missing ones as not graded.
      aligned = []
    ...
    logger_.error(f"Error reviewing words for conversation {id}: {str(e)}")
    return jsonify({'message': 'Internal server error'}), 500
